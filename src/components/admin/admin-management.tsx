...
export function AdminManagement({ currentUserEmail }: { currentUserEmail: string }) {
    const { data: admins, error, mutate, isLoading } = useSWR<AdminUser[]>('/api/admin/management', fetcher);
            ...
            <CardContent className="space-y-6">
                ...
                <div className="border rounded-md">
                    <Table>
                        ...
                        <TableBody>
                            {isLoading && (
                                <TableRow><TableCell colSpan={4} className="h-24 text-center">Loading admins...</TableCell></TableRow>
                            )}
                            {error && (
                                <TableRow><TableCell colSpan={4} className="h-24 text-center text-destructive">Could not load admins.</TableCell></TableRow>
                            )}
                            {admins && admins.map((admin) => {
                                const isCurrentUser = admin.email === currentUserEmail;
                                const canRevoke = !isCurrentUser && admin.added_by_email !== currentUserEmail;

                                return (
                                    <TableRow key={admin.id}>
                                        <TableCell className="font-medium">{admin.email}{isCurrentUser && " (You)"}</TableCell>
                                        <TableCell>{admin.added_by_email || 'Initial Admin'}</TableCell>
                                        <TableCell>{new Date(admin.created_at).toLocaleDateString()}</TableCell>
                                        <TableCell className="text-right">
                                            <AlertDialog>
                                                <AlertDialogTrigger asChild>
                                                    <Button variant="ghost" size="icon" disabled={!canRevoke} title={canRevoke ? "Revoke access" : "Cannot revoke this user"}>
                                                ...
                                                <AlertDialogContent>
                                                    ...
                                                    <AlertDialogFooter>
                                                        ...
                                                        <AlertDialogAction onClick={() => handleRevokeAdmin(admin.email)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
