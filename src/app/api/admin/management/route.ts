...
// GET: Fetch admins and identify the true initial admin
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        `;
        // Find the one true initial admin (the one with the lowest ID / first created)
        ...
        const [adminsRes, initialAdminRes] = await Promise.all([
            ...
            pool.query(initialAdminQuery)
        ]);

        const initialAdminEmail = initialAdminRes.rows.length > 0 ? initialAdminRes.rows[0].email : null;

        // Return both the list of admins and the email of the initial admin
...
// DELETE: Revoke admin with correct initial admin checks
export async function DELETE(req: NextRequest) {
    ...
    try {
        ...
        // Find the true initial admin (by lowest ID)
        const initialAdminRes = await client.query(`SELECT email, id FROM admin_users ORDER BY id ASC LIMIT 1;`);
        if (initialAdminRes.rows.length === 0) {
            throw new Error("System error: No initial admin found.");
        }
        const initialAdmin = initialAdminRes.rows[0];

        // Rule: You cannot revoke the initial admin
        ...
        // Rule: You cannot revoke your inviter
        ...
        return NextResponse.json({ success: true, message: `Access for ${emailToRevoke} has been revoked.` });
    } catch (error) {
        await client.query('ROLLBACK');
