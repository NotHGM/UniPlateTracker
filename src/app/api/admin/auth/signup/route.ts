...
export async function POST(req: NextRequest) {
    ...
    if (!email || !password) return new NextResponse(JSON.stringify({ message: 'Email and password are required.' }), { status: 400 });
    const lowerCaseEmail = email.toLowerCase();
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const approvedRes = await client.query('SELECT id FROM approved_emails WHERE email = $1', [lowerCaseEmail]);
        if (approvedRes.rows.length === 0) return new NextResponse(JSON.stringify({ message: 'This email is not approved.' }), { status: 403 });
        const existingUserRes = await client.query('SELECT id FROM admin_users WHERE email = $1', [lowerCaseEmail]);
        if (existingUserRes.rows.length > 0) return new NextResponse(JSON.stringify({ message: 'Account already exists.' }), { status: 409 });
