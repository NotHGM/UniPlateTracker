...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        if (userCheckRes.rows.length === 0 || userCheckRes.rows[0].added_by !== null) {
            return new NextResponse(JSON.stringify({ error: 'Access Denied: This log is for the initial administrator only.' }), { status: 403 });
        }
        ...
        const query = `
            ...
            LIMIT 50; -- Limit to the last 50 activities to keep it snappy
        `;
